...
const API_BASE_URL = 'http://localhost:3000/api/picks';
...
const Soccer = () => {
    ...
    const fetchPicks = useCallback(async (date) => {
        ...
        // --- MOCK DATA para garantir que funciona sem API real ---
        const mockPicks = {
            '2025-11-04': [
                { id: 'mock-p1', league: 'Premier League', homeTeam: 'Arsenal', awayTeam: 'Tottenham', dateTime: '16:00', tip: 'Over 2.5 Gols', odds: 1.85, confidence: 75, result: 'pending', analysis: 'Análise mock...' },
                { id: 'mock-p2', league: 'Serie A', homeTeam: 'Juventus', awayTeam: 'Inter', dateTime: '18:30', tip: 'BTTS', odds: 1.70, confidence: 82, result: 'pending', analysis: 'Análise mock...' },
            ],
            '2025-11-05': [],
        };
        ...
        try {
            ...
            if (data.data && data.data.length > 0) {
                ...
                setPicksData(mockPicks[date] || []); // Fallback para mock data se a API estiver vazia
                if (!mockPicks[date] || mockPicks[date].length === 0) {
                    setError(data.message || 'Nenhum palpite encontrado para a data.');
                }
            }
        } catch (err) {
            // Se falhar a API (ou timeout), usa o mock
            console.error("Erro ao buscar dados da API. Usando Mock Data:", err);
            const data = mockPicks[date] || [];
            setPicksData(data);
            if (data.length === 0) {
                setError(`Não foi possível conectar à API. (Usando mock data, mas não há dados para ${date}).`);
            } else {
                 setError(null); // Limpa o erro se o mock data tiver sucesso
            }
        } finally {
            setIsLoading(false);
        }
