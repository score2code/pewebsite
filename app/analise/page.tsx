    ...
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {games.map((game, index) => (
          <Link key={`${game.homeTeam}-${game.awayTeam}-${game.date}`} href={`/analise/${game.date}/${slugify(game.homeTeam)}-x-${slugify(game.awayTeam)}`} className={`block rounded-xl p-4 border ${borderColors[index % borderColors.length]} bg-light-100/50 dark:bg-dark-800/50 shadow-custom dark:shadow-custom-dark hover:shadow-lg hover:border-purple-500 dark:hover:border-purple-400 transition-all duration-300 ease-in-out transform hover:-translate-y-1`}>
