...
import RelatorioPunterClient from './RelatorioPunterClient';
...
export const metadata: Metadata = {
  title: 'Relatório Punter (Privado)',
  description: 'Relatório baseado em registros de punter.',
  robots: { index: false, follow: false },
};

async function loadBets(): Promise<BetRow[]> {
  const dirCandidates = [
    path.join(process.cwd(), 'app', 'data', 'hidden', 'punter'),
    path.join(process.cwd(), 'data', 'hidden', 'punter'),
  ];
  ...
  const fileFallbacks = [
    path.join(process.cwd(), 'app', 'data', 'hidden', 'punter.json'),
    path.join(process.cwd(), 'data', 'hidden', 'punter.json'),
  ];
    ...
    <div className="min-h-screen pt-8 pb-16 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <header className="bg-light-100/50 dark:bg-dark-800/50 rounded-xl p-6 md:p-8 mb-6 md:mb-8 border border-light-300 dark:border-dark-600 shadow-custom dark:shadow-custom-dark backdrop-blur-sm">
          <h1 className="text-3xl md:text-4xl font-bold text-dark-900 dark:text-light-100 mb-2">Relatório – Punter</h1>
          <p className="text-base md:text-lg text-dark-900/70 dark:text-light-100/70">Baseado em dados de punter.</p>
        </header>

        <RelatorioPunterClient bets={bets} initialBankroll={initialBankroll} />
      </div>
    </div>
  );
}
