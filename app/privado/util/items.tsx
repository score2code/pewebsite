...
import { Banknote, TrendingUp, Target, ListChecks, XCircle, CheckCircle } from 'lucide-react';
...
export default async function Items({ category }: { category: string }) {
  ...
  const totalBets = () => bets.filter(b => b.type === 'bet').length;
  const totalReds = () => bets.filter(b => b.status === 'red').length;
  const totalGreens = () => bets.filter(b => b.status === 'green').length;
  const totalStake = () => bets.filter(b => b.status !== 'pending').reduce((sum, b) => sum + (b.stake || 0), 0);
  const totalReturn = () => bets.filter(b => b.status !== 'pending').reduce((sum, b) => sum + (b.return || 0), 0) - totalStake();

  const returnClass = `${totalReturn() > 0 ? 'text-green-700 dark:text-green-400' : totalReturn() < 0 ? 'text-red-700 dark:text-red-400' : 'text-dark-900/70 dark:text-light-100/70'}`
    ...
    <div className="grid grid-cols-2 md:grid-cols-6 gap-3 mb-2">
      ...
      <div className="rounded-lg border border-light-300 dark:border-dark-600 bg-light-100/50 dark:bg-dark-800/50 p-3 flex items-center gap-2">
        ...
        <div className="flex-1">
          ...
          <div className={`font-bold ${returnClass}`}>{formatCurrencyBRL(totalReturn())}</div>
        </div>
      </div>
